...
public class MockedQueue extends RequestQueue {
    ...
    void deliverResponse(Request<?> request) {
      ...
      try {
        if (config.rawResponse != null) {
          // BasicNetwork : http status 200-299 -> returns NetworkResponse, handled via
          // parseNetworkResponse
          //                other status code   -> returns ServerError, handled via
          // parseNetworkError
        ...
        throw new RuntimeException(
            "Invalid response type. Request '"
                + request.getUrl()
                + "' ("
                + request.getClass().getName()
                + ") did not accept response '"
                + parsedResponse
                + "' ("
                + name
                + ")");
  ...
  public static class RuleConfiguration {
    ...
    /**
     * Add a filter to only accept requests with url (fully) matches {@code url}. Only the scheme,
     * domain and path is matched, query parameters are ignored.
     *
    ...
    public RuleConfiguration whenUrl(final String url) {
      return whenRequest(
          new RequestFilter() {
            @Override
            public boolean apply(Request<?> request) {
              return Uri.parse(request.getUrl()).buildUpon().clearQuery().toString().equals(url);
            }
          });
    }

    /**
     * Add a filter to only accept requests with url partially matching {@code path}. Only the
     * scheme, domain and path is matched, query parameters are ignored.
     *
    ...
    public RuleConfiguration whenPath(final String path) {
      return whenRequest(
          new RequestFilter() {
            @Override
            public boolean apply(Request<?> request) {
              return Uri.parse(request.getUrl()).buildUpon().clearQuery().toString().contains(path);
            }
          });
    }
    ...
    /**
     * If set, then this configuration will be applied to all request. (default behaviour is that
     * the configuration is only applied to the first matching request and then discard)
     *
     * @return configuration builder
     */
    ...
    /**
     * Mark this rule as optional. Optional rules will be excluded during verification using {@link
     * #verify()}. By default all rules are mandatory and will be check during verification.
     *
     * @return configuration builder
     */
   ...
   *
   * @return new rule configuration: Use {@code whenXxx()} to specify what requests should be
   *     matched and use {@code returnXxx()} to then specify what response should be delivered to
   *     those requests By default a rule will only be applied once, use {@code always()} to apply
   *     it more than once.
   */
  ...
  /**
   * Checks if all configured rules have been used to service a queued request. If any rule has
   * never matched any network request and is not marked as optional, then this method will an
   * AssertionError().
   *
  ...
  /**
   * Checks if there is a appropriate response set up for the request. If found, the response will
   * be pushed to the request and the response will be removed from {@link #rules}. Otherwise, a
   * {@link TimeoutError} will be delivered as response.
   */
